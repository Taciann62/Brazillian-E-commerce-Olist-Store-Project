...
-- To join the datasets containing customer data and orders together
...
-- Subtract the time between order placement and delivery to figure out wait time or duration.
...
-- Subquery/ Temporary table to further query the data for easy identification of Days of week, Month, and Year.
...
-- Insights into the sales by customer state to derive SUM, AVG, SDV, and FREQUENCY
...
-- DURATION BETWEEN ORDER APPROVAL AND PURCHASE
SELECT customer_state,
       order_approved_year,
...
-- BIND CUSTOMER DATA AND GEOLOCAION
